...
Oleh karena itu dalam project kali ini saya sebagai _data science_ diminta oleh perusahaan _Airbnb_ untuk membuat _model machine learning_ yang mampu memprediksi harga sewa yang cocok terhadap suatu hotel airbnb berdasarkan data-data yang ada seperti lokasi, nama, dan lain sebagainya.
...
Beberapa solusi yang bisa diterapkan untuk melatih model bisa dengan menggunakan lebih dari satu algoritma seperti, K Nearest Neighbors, Boosting Algorithm, dan Random Forest dan menentukan algoritma mana yang paling akurat dan terbaik untuk menentukan harga sewa, serta mengukur nilai atau tingkat akurasi model menggunakan metrik Mean Square Root
...
2. Data Cleaning:
   ...
   - Karena sudah dalam bentuk dataframe kita bisa menghapus kolom yang tidak relevan atau tidak digunakan dengan memanggil method _drop_ dalam analisis, seperti 'id', 'host_id', 'name', 'host_name', dan 'last_review' yang tidak memberikan pengaruh dalam pemodelan harga sewa .

   - Melakukan input nilai yang _null_ pada kolom _review per month_ dengan nilai rata-rata dari kolom yang sama tidak bisa dilakukan penghapusan karena jumlah record yang memiliki _NaN value_ terdapat 10052 record sehingga jika di-drop pasti akan mempengaruhi hasil model.
...
1. K-Nearest Neighbors (KNN):
   ...
   - Adapun kelebihan dari algoritma ini adalah mudah dipahami dan dapat digunakan untuk masalah regresi.

   - Kekurangan dari algoritma ini adalah pemilihan nilai K yang tepat menjadi penting dalam performa algoritma ini.

2. _Random Forest_:

   - Algoritma ensemble yang terdiri dari banyak _decision tree_ sehingga membentuk hutan  _(forest)_ dimana setiap pohon dipelajari pada data acak dan kemudian dicari rata-ratanya atau kadang sering disebut juga menggunakan konsep _bagging_.
